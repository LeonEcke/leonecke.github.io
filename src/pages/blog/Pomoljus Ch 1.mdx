...
As the Pomoljus is a physical object, it's also constrained by physical laws. For electricity to flow from the battery into the LEDs there needs to be an electrically conductive connection between them.
The standard for this is copper <sup>*(or gold for more demanding scenarios)*</sup> stuck to a glass-fiber sheet. This is what is called a printed circuit board, or PCB. But you can't just connect a power source to a light, as the amount of current that flows through the circuit is directly proportional to the resistance in said circuit. Higher currents leads to higher wattages which leads to heat which leads to a need for replacement components.
Sadly enough for us, the copper traces on the PCB <sup>*(and most of our components)*</sup> are of quite low resistance which would give us a current so high that the components would go poof.
